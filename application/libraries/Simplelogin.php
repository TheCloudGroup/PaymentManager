...
class Simplelogin
{
	...
	function create($user = '', $password = '', $auto_login = true) {
		...
		$this->CI->db->where('Provider_Name', $user);
		...
		if ($query->num_rows() > 0) {
			...
			if($auto_login) {		
				...
				//Set session data
				$this->CI->session->set_userdata(array('id' => $user_id,'Provider_Name' => $user));
	...
	function login($user = '', $password = '') {
		...
		if($this->CI->session->userdata('Provider_Name') == $user) {
		...
		if ($query->num_rows() > 0) {
			...
			if(md5($password) != $row['Provider_Password']) {
				return false;
			}
